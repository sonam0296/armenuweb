...
class Setting extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      loginContent: [],
      site_content: [],
      addUpdateContentModal: false,
      addUpdateContentModalFlag: false,
  ...
  openAddUpdateModal = () => {
    this.setState({
      status: "add",
      addUpdateContentModal: true,
      addUpdateContentModalFlag: true
    })
  }
  CloseAddUpdateModal = () => {
    this.setState({
  ...
  deleteSiteContent = (data, i) => {
    let { site_content, msg } = this.state;
    site_content.splice(i, 1);
    this.setState({
  ...
  getDefualtImage = async () => {
    const response = await instance.get(requests.fetchAllDefaultImage, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
    ...
    if (response && response.data) {
      this.setState({
        restuarantCoverImageprev: response.data.data.hasOwnProperty("restaurant_cover_image") ? `${response.data.data.restaurant_cover_image.image_url}` : `${response.data.data.default_image.image_url}`,
        siteLogoprev: response.data.data.hasOwnProperty("site_logo") ? `${response.data.data.site_logo.image_url}` : `${response.data.data.default_image.image_url}`,
        searchCoverImageprev: response.data.data.hasOwnProperty("search_cover") ? `${response.data.data.search_cover.image_url}` : `${response.data.data.default_image.image_url}`,
        defaultImageprev: response.data.data.hasOwnProperty("restaurant_default_image") ? `${response.data.data.restaurant_default_image.image_url}` : `${response.data.data.default_image.image_url}`,
  ...
  getSiteSettingData = async () => {
    ...
    if (response && response.data) {
      this.setState({
        ...
        site_content: response.data.data.siteInfo.site_content
      });
    }
  ...
  getLoginContent = async () => {
    ...
    const response = await instance
      .post(requests.fetchLoginPageInfo, SiteId, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
    ...
    if (response && response.data) {
      console.log("LOGIN Page Info ====> ", response);
    }
  }
  ...
  handleFileChangedefaultImage = (e) => {
    ...
    this.setState(
      { defaultImageData: data, defaultImage: URL.createObjectURL(data) }
    );
  ...
  handleFileChangesiteLogo = (e) => {
    ...
    const data = e.target.files[0];
    this.setState(
      { siteLogoData: data, siteLogo: URL.createObjectURL(data) }
    );
  };
    ...
    return (
      <>
        ...
        <Container className="pt-7" fluid>
              ...
              <Card className="shadow">
                <CardHeader className="border-0">
                  <div className="d-flex justify-content-between">
                    <div className="md-7">
                      <h1 className="mb-0">{i18next.t("Seetings Management")}</h1>
                    </div>
                    <div className="md-5">
                      <Row>
                        <Col></Col>
                ...
                <CardBody>
                  ...
                  >
                    <NavItem>
                      <NavLink
                        ...
                        className={classnames("mb-4 sm-4 xs-12 lg-4 xl-4", {
                  ...
                  >
                      ...
                      <Form>
                        <Row>
                          <Col md="12">
                            <div className="d-flex justify-content-between">
                              <div className="md-7">
                                <h1 className="mb-0"> {i18next.t("Content")}</h1>
                              </div>
                              <div className="md-5">
                                <Row>
                                  <Col>
                                    <Button size="sm" color="primary"
                                      onClick={this.openAddUpdateModal}
                                      type="button">
                            ...
                            <FormGroup>
                              ...
                              <Table className="mt-3 align-items-center table-flush" responsive>
                                {
                                  site_content.map((data, i) =>
                                    <>
                                      <tr>
                                        <td>
                                          <Input
                                        ...
                                        <td>
                                          <Button
                                            ...
                                            color="success"
                                            size="sm"
                                            onClick={() => this.openEditUpdateModal(data, i)}
                                          >
                                          ...
                                          <Button
                                            style={{ fontSize: "1rem" }}
                                            onClick={() => this.deleteSiteContent(data, i)}
                                            color="danger"
                                            size="sm"
                                          >
                    ...
                    <TabPane tabId={3}>
                      <Row>
                        <Col md={6} lg={4} sm={6} xs={12} xl={4}>
                          <FormGroup className="text-center font-weight-bold mb-12">
                            <Label for="input-name">{i18next.t("Site Logo")}</Label>
                            <div>
                              <div
                        ...
                        <Col md={6} lg={4} sm={6} xs={12} xl={4}>
                          <FormGroup className="text-center font-weight-bold mb-12">
                            <Label for="input-name">{i18next.t("Search Cover")}</Label>
                            <div>
                              <div
                                className="fileinput fileinput-new"
                                dataprovider="fileinput"
          ...
          {
            this.state.addUpdateContentModalFlag === true &&
            <LoginContent
              show={this.state.addUpdateContentModal}
              onClose={this.CloseAddUpdateModal}
              site_content={this.state.site_content}
              getSiteSettingData={this.getSiteSettingData}
