...
// Alert
import AlertInsertedSuccess from "../../../common/AlertBox/alertBoxInsertSuccess";
...
class DriverRegistration extends React.Component {
  ...
  getRestaurantDetail = async () => {
    ...
    const response = await instance.get(requests.fetchGetRestaurantForAddDriver,{
      headers:{
    ...
    if(response && response.data){
      ...
      const CData =
          response.data.data != null
            ? response.data.data.map((Cd, i) => {
                ad[i] = {
                  value: Cd._id,
                  key: Cd._id,
                  text: Cd.restaurant_Name,
                };
              })
            : [{ value: "", text: "", key: "" }];
      this.setState({
    ...
    this.setState({
      ...
      this.state.pathStatus==="edit" ?
        this.setState({
          ...
          restaurant: EditDriverDetail.employer_id,
    ...
    .then(response => {
      this.setState({
        ...
        const response = this.state.pathStatus==="create" ?
            await instance.post(requests.fetchAddDriverFromAdmin,bodyAPI,{
              headers:{
                "Authorization":`Bearer ${token}`
              }
            })
          :
          await instance.patch(requests.fetchUpdateDriverFromAdmin, bodyAPI, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
              ...
              <Card className="bg-secondary shadow">
                ...
                <CardBody>
                  <Form>
                    ...
                    {
                      this.state.alertInserted===true &&
                        <AlertInsertedSuccess />
                    }
                  </Form>
                </CardBody>
              </Card>
