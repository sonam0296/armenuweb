...
const mapStateToProps = (state) => {
  hostingAddress = state.hostingAddress;
  ownerId = state.ownerId;
  dishDataRedux = state.dishData;
  userData = state.userData;
  token = state.token;
  Cart_List = state.Cart_List===null ? {} : state.Cart_List
};
...
class Restaurant extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      myCart: Cart_List.hasOwnProperty("myCart")? Cart_List.myCart : [] ,
      sub_total: Cart_List.hasOwnProperty("sub_total")?  Cart_List.sub_total :  0,
  ...
  GetRestaurantDetail = async () => {
    ...
    let ownerDetail = {
      owner_id: `${ownerId}`,
      menu_category_id: "",
      hosting_Address: hostingAddress //"dummystripe.appetizar.io", //"dummyrazorpay.appetizar.io"
    };
  ...
  setCartValue = () => {
    if(Cart_List===undefined){
      ...
      this.setState({
        sub_total:Cart_List.sub_total,
        myCart:Cart_List.myCart
      })
    }
  }
    ...
    if (token) {
      if(Cart_List.hasOwnProperty("owner_id")){
        if(ownerId === Cart_List.owner_id ){
          ...
          if (response && response.data) {
            successToaster('Successfully Added Item..!');
        ...
        let item_list = this.state.myCart===undefined ? [] : this.state.myCart
        item_list.push(item);
        this.setState({
          myCart: item_list
        })
        ...
        this.setState({
          sub_total: this.state.sun_total===undefined ? 0 : this.state.sub_total
      })
      ...
      errorToaster("Logged in is Required..!");
    }
  }
  onDismiss = () => {
    this.setState({
  ...
  render() {
    ...
    let restaurant_cover_image = this.state.resturantDetail.restaurant_cover_image ? this.state.resturantDetail.restaurant_cover_image.image_url : process.env.REACT_APP_DEFAULT_IMAGE
    return (
      <>
        <HomeHeader />
        <div
                                    ...
                                    <div className="strip" style={{padding:'15px'}}>
                                      <Col>
                                        <Row>
                                          ...
                                          >
                                            <img
                                              src={item.item_image ? item.item_image.image_url : process.env.REACT_APP_DEFAULT_IMAGE}
                                              loading="lazy"
                    ...
                    <Col md={12} lg={12} xs={12} xl={12}>
                        <div className="text-left">
                          <figure className="lazy">
                            <img
                              src={this.state.singleDishItem.item_image ? this.state.singleDishItem.item_image.image_url : process.env.REACT_APP_DEFAULT_IMAGE}
                              loading="lazy"
                              data-src="/default/restaurant_large.jpg"
                              className="lazy"
                              alt=""
