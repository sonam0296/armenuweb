...
import UpdateDishOutlet from "./UpdateDishOutlet";
...
export class UpdateItem extends Component {
  ...
  handleDeleteDish = async (id) => {
    ...
    if (response && response.data) {
      successToaster("Item successfully deleted!");
  ...
  handleUpdateDishItem = async () => {
	...
	errorToaster("Item Name Field is required")
}
    if (item_description.length > 0) {
      fd.append("item_description", item_description);
    }
 else{
	errorToaster("Item Description Field is required")
}
    if (item_price != null) {
      fd.append("item_price", item_price);
    }
 else{
	errorToaster("Item Price Field is required")
}
    if (vat_percentage != null) {
      fd.append("vat_percentage", vat_percentage);
    }
    else{
	errorToaster("Item Percentage Field is required")
}
    fd.append("item_available", this.state.item_available);
    fd.append("enable_variants", this.state.enable_variants);
    fd.append("recommend", this.state.recommend);
    fd.append("veg", this.state.veg);
    ...
    const response = await instance
      ...
      .catch((error) => {
        // let errorMessage = error.response.data.error.message;
        // if(errorMessage===`"vat_percentage" is required`)
        // {
        //   errorToaster("Tax percentage is reqired");
        // }else{
        //   errorToaster(errorMessage);
        // }
        console.log(error);
    ...
    if (response && response.data) {
      ...
      successToaster("Record updated successfully");
      this.setState(
        {
          image: "",
          item_image: "",
          ...
          <Container className="pt-7" fluid>
            <Row>
                      ...
                      <Form>
                        ...
                        <FormGroup>
                          <Label
                            ...
                            for="vat_percentage"
                          >
                            {i18next.t("Tax percentage( calculated into item price )")}
                          </Label>
                          <Input
                            className="px-2 py-4"
                            type="number"
                            placeholder={i18next.t("Item tax_percentage")}
                            name="vat_percentage"
                            value={vat_percentage}
                            onChange={(e)=>this.handleChangeAll(e)}
                            min="0"
                            invalid={vat_percentage < 0}
                          />
                          <FormFeedback invalid>
                            Uh oh! please give a valid price.
                          </FormFeedback>
              ...
              <Col xs={12} sm="12 mb-4" xl={6} className="card-left ">
                ...
                <Card className="mt-4 bg-profile shadow">
                  <CardHeader className="bg-white border-0">
                    <Row className="align-items-center">
                      <Col xs="8">
                        <h3 className="mb-0">{i18next.t("Outlets")}</h3>
                      </Col>
                      <Col className="text-right" xs="4">
                        {/* <Button
                          color="primary"
