...
class Checkout extends React.Component {
  ...
  handleDateOpen = (date) => {
    ...
    if (CampareDate > newDate) {
      errorToaster("Please Enter Correct Time....");
  ...
  onCallIncreaseQuantity = async (data, i, price) => {
    ...
    if (cart.myCart[i].menu_item_qty < 10) {
      ...
      errorToaster("You can't more than 10 quantity ");
    }
  ...
  onCallDecreseQuantity = async (data, i, price) => {
    ...
    if (cart.myCart[i].menu_item_qty > 1) {
      ...
      errorToaster("You can't decraese the quantity ");
    }
  };
  ...
  ClearCart = async () =>{
    ...
    if (response && response.data) {
      //successToaster("Order is placed...!");
      this.props.GET_LIST_CART(response.data.data);
      this.getData();
  ...
  placeOrder = async (e) => {
    ...
    if (this.state.CashOnDelivery === "true") {
      ...
      const response = await instance
      .post(requests.fetchCashOrder ,bodyAPI, {
        headers: {
        Authorization: `Bearer ${token}`,
        },
      })
      ...
      if (response && response.data) {
        successToaster("Your Order Succesfully Placed...");
        setTimeout(() => {
          this.setState({
      ...
      if(userData.country_name === "India")
      {
        ...
        if (response && response.data) {
          ...
          this.setState({
            ...
            raz_account_id: response.data.data.raz_account_id
          })
          this.ShowRazorPay();
        }
      }
  ...
  ShowRazorPay = () => {
    ...
    var options = {
      ...
      "account_id": this.state.raz_account_id,
      ...
      handler: function (response) {
        ...
        if(response.razorpay_signature){
          ...
          successToaster("Your Order Successfully Placed...");
          this.ClearCart();
          this.setState({
            redirectIfCod:true
          })
                          ...
                          <>
                            <Row style={{ borderRight: "5px solid #5e72e4" }}>
                              ...
                              >
                                <div>
                                  <img
                                    src={data.menu_item_id.item_image ? data.menu_item_id.item_image.image_url : process.env.REACT_APP_DEFAULT_IMAGE}
                                    width="100"
                                    height="105"
                                    alt=""
